...
import Achievements;
import StageData;
//import ModchartState; //KE and BETADCIU support -- Will probably add later
import FunkinLua;
import DialogueBoxPsych;
import Conductor.Rating;
import datas.RatingsData;
...
class PlayState extends MusicBeatState
{
	...
	public var characterLuas:Map<String, FunkinLua> = new Map();
	...
	var barSongLength:Float = 0; //Hey you can now do fake-outs ig
	...
	override public function create()
	{
		...
		if (ClientPrefs.timeBarStyle == 'Indie Cross') {
		...
		if (ClientPrefs.timeBarStyle == 'Default') {
		...
		var splashOpp:NoteSplash = new NoteSplash(100, 100, 0);
		grpNoteSplashes.add(splashOpp);
		splashOpp.alpha = 0.0;
		...
		if (ClientPrefs.smoothHP) {
			healthBar = new HealthBar(0, FlxG.height * (!ClientPrefs.data.downScroll ? 0.89 : 0.11), 'healthBar', function() return fakeHealth, 0, 2);
		} else {
	...
	override public function update(elapsed:Float)
	{
		...
		if (ClientPrefs.smoothHP)
			{
			var mult = FlxMath.lerp(fakeHealth, health, ((health / fakeHealth) * (elapsed * 8)) * playbackRate);
			//healthBar.numDivisions == 10000; //This seems it's a bit too high maybe
			fakeHealth = mult;
			}
		...
		else
		{
			if (!paused)
			{
				...
				if(updateTime) {
					...
					secondsTotal = Math.floor((songCalc/playbackRate) / 1000);
					...
					if(ClientPrefs.timeBarType != 'Song Name' && barSongLength <= 3600000)
						timeTxt.text = FlxStringUtil.formatTime(secondsTotal, false);

					if(ClientPrefs.timeBarType != 'Song Name' && barSongLength >= 3600000)
					timeTxt.text = hoursRemaining + ':' + minutesRemainingShit + ':' + secondsRemaining;

					if(ClientPrefs.timeBarType == 'Modern Time' && barSongLength <= 3600000)
					...
					if(ClientPrefs.timeBarType == 'Song Name + Time' && barSongLength <= 3600000)
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		if (stageUI != "normal")
		{
			uiPrefix = '${stageUI}UI/';
			if (PlayState.isPixelStage) uiSuffix = '-pixel';
			var antialias = !isPixelStage;
		}

		rating.loadGraphic(Paths.image(uiPrefix + daRating.image + uiSuffix));
		if (ClientPrefs.ratingCamType == 'camGame') {
			rating.cameras = [camGame];
		} else {
		if (ClientPrefs.ratingCamType == 'camHUD') {
			rating.cameras = [camHUD];
		} else {
			if (ClientPrefs.ratingCamType == 'camOther') {
		...
		FlxTween.tween(rating, {angle: 0}, 0.2 / playbackRate, {
			startDelay: Conductor.crochet * 0.001 / playbackRate
		});
	...
	function opponentNoteHit(note:Note):Void
	{
		...
		if (!note.isSustainNote)
			{
				if (!note.noteSplashDisabled && ClientPrefs.oppNoteSplashes)
					{
	...
	public function spawnOpponentNoteSplashOnNote(note:Note)
		{
			if (ClientPrefs.noteSplashes && note != null)
			{
				...
				if (strum != null)
				{
					spawnOpponentNoteSplash(strum.x, strum.y, note.noteData, note);
				}
			}
		}
	...
	public function spawnOpponentNoteSplash(x:Float, y:Float, data:Int, ?note:Note = null) {
		...
		if (data > -1 && data < ClientPrefs.arrowHSV.length)
		{
			...
			brt = ClientPrefs.arrowHSV[data][2] / 100;
			if(note != null) {
				skin = note.opponentNoteSplashTexture;
				hue = note.noteSplashHue;
				sat = note.noteSplashSat;
				brt = note.noteSplashBrt;
			}
		...
		var splashOpp:NoteSplash = grpNoteSplashes.recycle(NoteSplash);
		splashOpp.setupNoteSplash(x, y, data, skin, hue, sat, brt);
		grpNoteSplashes.add(splashOpp);
	}
	...
	public function RecalculateRating(badHit:Bool = false) {
		...
		if(ret != FunkinLua.Function_Stop)
		{
			...
			else
			{
				...
				var ratings:Array<Dynamic> = RatingsData.psychRatings;
			...
			// Rating FC
			ratingFC = "";
			if (sicks > 0) ratingFC = "MFC"; //Fuck you Rab lol /pos
